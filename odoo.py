...
def upsert_person(uid: int, person_id: int, force: bool = False) -> int:
    ...
    vals = {"name": name, "is_company": False, "company_type": "person"}

    # Only write fields if they have actual values — never overwrite existing data with None
    if parent_id:
    ...
    odoo_id = odoo_create(uid, "res.partner", vals)
    odoo_write(uid, "res.partner", odoo_id, {"is_company": False, "company_type": "person"})
    mapping_set("person", person_id, odoo_id)
    return odoo_id
